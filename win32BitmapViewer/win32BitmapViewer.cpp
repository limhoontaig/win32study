// win32BitmapViewer.cpp : 애플리케이션에 대한 진입점을 정의합니다.
//
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
    ...
    wcex.hIcon          = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_WIN32BITMAPVIEWER));
    ...
    case WM_COMMAND:
        ...
        static char strFileExtension[10];
        {
            ...
            case ID_32771:
                ...
                {
                    hBitmap = (HBITMAP)LoadImage(NULL, strFile,
                        IMAGE_BITMAP, 0, 0, 
                        LR_CREATEDIBSECTION | LR_LOADFROMFILE | LR_DEFAULTSIZE);
                    GetObject(hBitmap, sizeof(BITMAP), &BitmapInfo);
                    nWidth = BitmapInfo.bmWidth;
                    nHeight = BitmapInfo.bmHeight;
                    hdc = GetDC(hWnd);
                    if (hMemDC != NULL)
                        DeleteDC(hMemDC);
    ...
    case WM_PAINT:
        {
            ...
            if (hMemDC != NULL)
            {
                BitBlt(hdc, 50, 50, nWidth, nHeight, hMemDC, 0, 0, SRCCOPY);
                DeleteDC(hMemDC);
            }
            EndPaint(hWnd, &ps);
        }
